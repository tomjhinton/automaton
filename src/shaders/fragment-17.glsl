...
vec4 sdScene(vec3 p) {
 ...
 // p.xy *= rot(vTime * .5 * -1.);
  ...
  vec4 sphereLeft = sdSphere(p3 + displacement  ,.8 , vec3(1., .0, .0), color1 );
  vec4 sphereRight = sdSphere(p3 , .8, vec3(-1.,.0, 0.), color1);
  vec4 sphereTop = sdSphere(p3 , .5, vec3(0., -1., -2), color3);
  vec4 sphereBot = sdSphere(p3 , .5, vec3(0., 1., -2), color1);
  ...
  vec4 link2 = sdLink(p3 + displacement2, .8, .15, .3 , color3);
  ...
  // float blah = mix(link, link2, sin(vTime * .5) * .5 +1.);
   ...
   vec4 mixed = minWithColor(sphereTop, sphereBot);
