...
vec4 sdScene(vec3 p) {
 ...
 // p.xy *= rot(vTime * .5 * -1.);
  ...
  vec4 sphereLeft = sdSphere(p3 + displacement3, 1.2, vec3(1., 0, -2), color1 + displacement4);
  vec4 sphereRight = sdSphere(p3 + displacement, sin(vTime) * .5 + 1., vec3(0., 0, -2), color2);
  vec4 sphereTop = sdSphere(p3 + displacement, .5, vec3(0., -1., -2), color3);
  vec4 sphereBot = sdSphere(p3 + displacement, .5, vec3(0., 1., -2), color4);
  vec4 cube = sdBox(p3 + displacement2  , vec3(1.) , color4 );
  vec4 cube2 = sdBox(p3   , vec3(1., sin(vTime) *.5 + 1., 1.) , color4 + displacement4);
  ...
  float blah = smoothDifferenceSDF(cube.r, cube2.r, .005);
